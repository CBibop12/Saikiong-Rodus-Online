...
const executeCommand = (object, context) => {
 ...
 if (matchState.teams[findCharacter(characterName).team].remain.actions === 0) {
  ...
  if (commandType === "buy") {
    ...
    let charObj = findCharacter(characterName);
  ...
  if (commandType === "attackBase") {
    ...
    if (!character) {
      addActionLog(`Не найден персонаж ${characterName}`);
      return;
    }
  ...
  if (commandType === "attack") {
    ...
    const attacker = findCharacter(characterName);
    let targetChar = findCharacter(target);
    if (!targetChar) {
  ...
  if (commandType === "useAbility") {
    ...
    const caster = findCharacter(characterName);
    if (!caster) {
      addActionLog(`Не найден персонаж ${characterName}`);
      return;
    }
