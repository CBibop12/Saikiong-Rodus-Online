...
export const calculateCellsZone = (startCoord, range, matchState, mapSize, selectedMap, forbiddenTypes = [], canGoThroughCharacters = false, canGoThroughObjects = false, teleportGoThrough = false) => {
  ...
  const freeCells = [] // Клетки-партнёры порталов, доступные без затрат ловкости
  ...
  if (reachableTeleports.length > 0 && teleportGoThrough) {
    for (let tp of reachableTeleports) {
      ...
      if (anotherPortal) {
        // Добавляем клетку-партнёр портала в freeCells (переход без затрат ловкости)
        if (!freeCells.includes(anotherPortal.coordinates)) {
          freeCells.push(anotherPortal.coordinates);
        }

        // Также добавляем в reachable, если её там нет
        if (!reachable.includes(anotherPortal.coordinates)) {
          reachable.push(anotherPortal.coordinates);
        }
